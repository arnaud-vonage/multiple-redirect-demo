...
`/answer`（着信応答）および `/event`（通話イベント）エンドポイントは公開 URL です。  
認証がなければ、URL を知っている第三者が任意のリクエストを送信できてしまいます。
...
| 攻撃 | 内容 | 防御方法 |
|------|------|----------|
| **不正 NCCO インジェクション** | 攻撃者が `/answer` に偽の Webhook を POST し、任意番号への発信・音声再生などを引き起こす | `api_application_id` 不一致で 401 を返し処理しない |
| **偽イベント注入** | 攻撃者が `/event` に偽の通話ステータスを POST し、通話ログや管理ダッシュボードを汚染する | 同上 |
| **なりすまし転送** | 攻撃者が `/answer` を呼び出して NCCO を取得し、転送先番号を特定する | 同上 |

#### 制限事項（クレームのみ検証の場合）

このアプリは JWT の **署名検証を行わず**、ペイロードのクレームのみを検証します。  
これは VCR の Webhook トークンが Vonage の公開 JWKS に含まれない内部鍵で署名されているためです。

そのため、エンドポイント URL と `api_application_id`（傍受した JWT のペイロードから読み取り可能）の両方を知る攻撃者は、理論上クレーム検証を回避できます。  
ただし、エンドポイント URL は VCR 内部にのみルーティングされるため、実際のリスクは低いと判断しています。

より強固な署名検証が必要な場合は、Vonage が VCR 向けの JWKS エンドポイントを公開しているか確認してください。
